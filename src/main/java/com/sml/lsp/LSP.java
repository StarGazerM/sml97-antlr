/*
* code is form https://github.com/georgewfraser/java-language-server
* TvT but I wipe all java 11 var lang feature.....
*
* */
package com.sml.lsp;
...
public class LSP {
    ...
    private static String readHeader(InputStream client) {
        ...
        for (char next = read(client); true; next = read(client)) {
            if (next == '\r') {
                char last = read(client);
                assert last == '\n';
                break;
            }
            line.append(next);
